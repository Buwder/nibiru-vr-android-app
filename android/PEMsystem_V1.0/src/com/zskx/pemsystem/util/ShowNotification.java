...
//		mNotificationManager.notify(123455,  mBuilder);
		...
		if(AppConfiguration.getUser(context) != null && !AppConfiguration.getUser(context).getSessionId().equals("")) {
			if(intent != null)  {
				pt = PendingIntent.getActivity(context, 12321, intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP), 0);
			}
			else if(clazz != null ){
				pt = PendingIntent.getActivity(context, 12321, new Intent(context, clazz).setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP), 0);
			}else{
				 pt = PendingIntent.getActivity(context1, 12321, new Intent(context1, LoginActivity.class).setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP), 0);
			}
		}else{
			 pt = PendingIntent.getActivity(context1, 12321, new Intent(context1, LoginActivity.class).setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP), 0);
		}
