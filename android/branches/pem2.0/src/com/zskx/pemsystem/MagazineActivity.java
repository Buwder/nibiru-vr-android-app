...
public class MagazineActivity extends MenuActivity {
	...
	private int default_max_type_num = 4; // number of typies
	...
	private void setClickListener() {
		...
		for (int i = 0; i < shelf_container.getChildCount(); i++) {
			...
			((ListView) v).setOnScrollListener(new OnScrollListener() {
				...
				public void onScroll(AbsListView view, int firstVisibleItem,
					...
					Log.i(TAG, "visibleItemCount:::" +  visibleItemCount + "    totalItemCount:::" + totalItemCount + "    firstVisibleItem:::" + firstVisibleItem);
					if (visibleItemCount > 0) {
						if (view.getLastVisiblePosition() + 1 >= totalItemCount) {

							String type_id = currentTab.getTag().toString();
							int pageNum = type_page.get(type_id);
							Integer pageTotalNum = type_page_total.get(type_id);
							if (pageTotalNum != null) {
								if (pageNum * PAGE_SIZE < pageTotalNum) {
									if (!(pageNum * PAGE_SIZE > data_map
											.get(type_id).size())) {
										Log.i(TAG, "翻頁加載！");
										type_page.put(type_id, pageNum + 1);
										getMagazineDataByTypeFromNet(type_id, false);
										Toast.makeText(MagazineActivity.this,
												"加载下一页！", 800).show();
									}
	 ...
	 */
	private void getMagazineDataByTypeFromNet(final String magazineTypeId,
			Boolean isLoadUi) {
		if (isLoadUi) {
