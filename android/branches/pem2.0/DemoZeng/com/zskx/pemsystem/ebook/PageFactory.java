...
public class PageFactory {
	...
	private float backGroundScaleH = 1.0f;
	private float backGroundScaleW = 1.0f;
	/** 字符编码 */
	...
	public void setBackGroundScale(float h,float w){
		this.backGroundScaleH = h;
		this.backGroundScaleW = w;
	}
	...
	public void drawLineWithBold(Canvas canvas, int y, String line,
		...
		for (Bold bold : list) {
			...
			/* 在这而需要考虑三个方面的情况:1.加粗字体被截为两行。2.加粗字体满行，被换行。3加粗字体超过两行的判断			 */
	...
	private void drawPage(Canvas canvas) {
		...
		if (mPageTxtLineSet.size() > 0) {
			...
			if (bgImage != null) {
				...
				matrix.setScale(backGroundScaleW, backGroundScaleH);
				canvas.drawBitmap(bgImage, matrix, mPaint);
			} else {
