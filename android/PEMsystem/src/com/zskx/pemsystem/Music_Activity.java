...
import com.zskx.pemsystem.adpater.PlayingListAdapter.PlayingListHolder;
...
public class Music_Activity extends Activity {
	...
	private ImageView music_img; //当前音乐图像
	private AsyncImageLoader asyncImageLoader = new AsyncImageLoader();;   //异步加载图像辅助类
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(AppConfiguration.user != null) {
			userId = AppConfiguration.user.getLoginName();
...
//		Log.i(TAG, "startWithPlaying, start!!  " + ckb_swp.isChecked());
	...
	private void getServerResponse() {
		 GetAllMusicTypeRequest allMusicTypeRequest = new GetAllMusicTypeRequest(new GetResponseListener<MusicTypeEntity>(){
	...
	private void afterResponse() {
		...
		mMusicListView.setAdapter(new MusicAdapter(Music_Activity.this, typeList , myHandler));
	...
	//	mPlaylist = new ArrayList<MusicDetailEntity>();
		...
		lyt_include_play = this.findViewById(R.id.music_include_play);
		...
		btn_play_menu = (Button) this.findViewById(R.id.music_button_play_menu);
		...
		btn_menu = (Button) this.findViewById(R.id.music_button_menu);
		
		lyt_play = this.findViewById(R.id.music_layout_play);
		lyt_list = this.findViewById(R.id.music_layout_playlist);
		lyt_playing = this.findViewById(R.id.music_layout_playing);
		lyt_menu = this.findViewById(R.id.music_layout_menu);
		...
		btn_volume.setOnClickListener(new BtnClick());
		...
		btn_play.setOnClickListener(new BtnClick());
		btn_prev.setOnClickListener(new BtnClick());
		btn_next.setOnClickListener(new BtnClick());
		...
		music_img = (ImageView) this.findViewById(R.id.music_playing_img);
		audioManager = (AudioManager) Music_Activity.this.getSystemService(AUDIO_SERVICE);
	
		server_progressBar =  findViewById(R.id.music_server_progress);
			...
			public void run() {
				MyPost.post(new Runnable() {
					...
					public void run() {
						...
						btn_playmode.setOnClickListener(new BtnClick());
...
//								PlayingListHolder listHolder = (PlayingListHolder) arg1.getTag();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case MusicTypeChildActivity.RESULTCODE:
			if(data != null) {
				...
				mPlaylist.addAll(cancleRepeat(musicList)); //从服务器音乐列表添加音乐到正在播放列表
			}
			break;
		}
	...
	BroadcastReceiver nextReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if ( mPlaylist != null && intent.getStringExtra("getMusicId") != null) {
...
//			mediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
	 ...
	 * 异步加载当前音乐图像
	 */
	protected void loadMusicImage(String name) {
		Log.i(TAG, "loadMusicImage,name==" + name);
		if (name!= null && !"".equals(name)) {
			asyncImageLoader.loadDrawable(name, new ImageCallback() {

				@Override
				public void imageLoaded(Drawable imageDrawable, String imageUrl) {
					music_img.setImageDrawable(imageDrawable);
				}
			});
		}
	}
...
//			System.out.println("allTimeShow:::" + mediaPlayer.getDuration());
	...
	private void setPlaymode(int flag_playmode) {
		switch (flag_playmode) {
		case StringsUtil.Circle:
			btn_playmode.setBackgroundResource(R.drawable.music_circle_pressed);
			break;
		case StringsUtil.Single:
			btn_playmode.setBackgroundResource(R.drawable.music_single_play_pressed);
			break;
		case StringsUtil.Sheffle:
			btn_playmode.setBackgroundResource(R.drawable.music_shuffle_pressed);
			break;
		case StringsUtil.Order:
			btn_playmode.setBackgroundResource(R.drawable.music_order_pressed);
			break;
		default:
			btn_playmode.setBackgroundResource(R.drawable.music_circle_pressed);
			break;
	...
	private final class LytClick implements View.OnClickListener{
		...
		public void onClick(View v) {
			switch (v.getId()) {
			case R.id.music_layout_play:
				lyt_include_play.setVisibility(View.VISIBLE);
				lyt_include_list.setVisibility(View.GONE);
				lyt_include_playing.setVisibility(View.GONE);
				
				lyt_play.setBackgroundColor(getResources().getColor(R.color.darkblue));
				lyt_list.setBackgroundColor(Color.TRANSPARENT);
				lyt_playing.setBackgroundColor(Color.TRANSPARENT);
				break;
			case R.id.music_layout_playlist:
				...
				lyt_include_play.setVisibility(View.GONE);
				lyt_include_list.setVisibility(View.VISIBLE);
				lyt_include_playing.setVisibility(View.GONE);
				
				lyt_list.setBackgroundColor(getResources().getColor(R.color.darkblue));
				lyt_play.setBackgroundColor(Color.TRANSPARENT);
				lyt_playing.setBackgroundColor(Color.TRANSPARENT);
			...
			case R.id.music_layout_playing:
				...
				lyt_playing.setBackgroundColor(getResources().getColor(R.color.darkblue));
				lyt_list.setBackgroundColor(Color.TRANSPARENT);
				lyt_play.setBackgroundColor(Color.TRANSPARENT);
				break;
			case R.id.music_layout_menu :
	
				popwindow_menu(v);
				break;
			
				
			case R.id.music_button_play_menu:
				lyt_include_play.setVisibility(View.VISIBLE);
				lyt_include_list.setVisibility(View.GONE);
				lyt_include_playing.setVisibility(View.GONE);
...
/*	private void showMenuDialog()
	...
	private void popwindow_menu(View parent){
		...
		btn_swp.setOnClickListener(new BtnClick());
		btn_mkp.setOnClickListener(new BtnClick());
		btn_fifo.setOnClickListener(new BtnClick());
...
/************************************************************/
	...
	Runnable FlashUI_Thread = new Runnable(){
		public void run() {
			...
			if(!isBufferReady && music_track != null){
				textView_tilte.setText("正在下载..." + music_track.getMusicTitle());
				title_progressBar.setVisibility(View.VISIBLE);
			}else{
				textView_tilte.setText(music_track.getMusicTitle());
			...
			if(music_img.getDrawable() == null ) {
				loadMusicImage(music_track.getMusicImage()); //如果图片没有加载，重新下载
				System.out.println("loadMusicImage");
			}
